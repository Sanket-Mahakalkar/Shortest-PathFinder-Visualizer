...
class PathFinder
{
	...
	public:
		...
		void ListOfHospitals();                                    // It shows the list centers
		
		void PrepareCityArea();                                    // It shows Graph of city
		
		void ShortestPathAlgo();                                   //  All pair shortest path algorithm
		
		void HighlightedShortestPath(vector<pair<int,int>>);      //It shows the highlighted shortest path
...
// It implements all pair shortest Path algorithm - floydWarshall

void PathFinder::ShortestPathAlgo()
{
...
// For the given source and destination it returns the shortest path

void PathFinder::result(char source,char dest)
{
	...
	cout<<"\n\n\t Have a look at highlighted GREEN colour path..!!";
...
// It highlights the shortest path from source to destination in GREEN color in the graph

void PathFinder :: HighlightedShortestPath(vector<pair<int,int>> res)
{
	...
	{
		line(res[i].first,res[i].second,res[i+1].first,res[i+1].second);
	}
	getch();			
}
...
// It prepares the layout of the city and hospitals

// CityArea is an adjacency Matrix which represent weights and source & destination of edges

void PathFinder :: PrepareCityArea()
{
...
// It allows the user to choose the source and destination from the list of centers 

void PathFinder::ListOfHospitals()
{
	while(1)
	{
		system("cls");
